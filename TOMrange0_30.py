...
output_name = '0_30outputSID%s.csv' % SID
...
def readInput():
	...
	cr = csv.reader(open("input0_30.csv", "rU"), dialect=csv.excel_tab)
	while True:
...
def unDecision(giveValue):
	...
	if giveValue >= 11 or (giveValue >= 5 and giveValue < 9):
			return False
	elif giveValue == 0:
		return True
	elif giveValue == 1:
		...
		return randgen < 20.0
	elif giveValue == 4 or giveValue == 9 or giveValue == 10:
		return randgen < 6.2	
	else:
...
rangeMin = 0 # lower range for this run, not currently used in logic
rangeMax = 30 # upper range for this run, just used for text on screen
